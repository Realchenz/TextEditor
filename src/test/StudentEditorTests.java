...
/**
 * This class represents the advanced test class for the text editor. You can run this.
 * Known Bugs: None
 *
 * @author Zhenxu Chen
...
class StudentEditorTests {
	 ...
	 * the cursor can be properly moved from before 'b' to before 'l' to before 'u'. And then move again to the very end, nothing happens.
	 */
	@Test
	void testMoveRightAdvanced() {
		...
		// move cur all the way to the end
		e.moveRight();
		...
		// move cur right again once it's at the end
		e.moveRight();
		assertEquals(4, e.getCursorPosition());
		assertNull(e.cur);
	}
	 ...
	 * the cursor can be properly moved from before 'e' to before 'u' to before 'l'. And then move again to the very beginning, nothing happens.
	 */
	@Test
	void testMoveLeftAdvanced() {
		// create editor storing "blue"
		loadEditorBLUE(e);
		...
		// move cur all the way to the beginning
		e.moveLeft();
		assertEquals(0, e.getCursorPosition());
		assertEquals('b', e.cur.data);

		// move cur left again once it's at the beginning
		e.moveLeft();
		assertEquals(0, e.getCursorPosition());
		assertEquals('b', e.cur.data);
	}
	 ...
	 * the Editor properly deletes 'l', then 'u', then 'e'. And then delete again, nothing happens.
	 */
	@Test
	 ...
	 * tests if the Editor properly backspaces 'u', then 'l', then 'b'. And then
	 * backspace again, nothing happens.
	 */
	@Test
	void testBackspaceMultipleMiddle() {
		...
		// backspace again, curPos stays at 0
		e.backspace();
	...
	/**
	 * Tests the clear() method described on page 7.
	 */
	@Test
	void testClear() {
		// create editor storing "blue"
